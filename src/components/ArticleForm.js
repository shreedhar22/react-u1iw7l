...
const mapDispatchToProps = dispatch => ({
  ...
  onSubmitForm: (title, description) => value =>
    dispatch({
      type: "CREATE_ARTICLE",
      payload: agent.Articles.createArticle(title, description)
    })
});
...
class ArticleForm extends React.Component {
  constructor() {
    ...
    this.onSubmitForm = (title, description) => event => {
      event.preventDefault(), this.props.onSubmitForm(title, description);
    };
  }
      ...
      <div>
        <form onSubmit={this.SubmitForm(title, description)}>
          ...
          <input
            type="string"
            id="description"
            onChangeDescription={/* TODO: add a onChange function */}
          />
